---
description: Verify OpenSpec requirements are properly implemented in tests
---

Verify that OpenSpec requirements are properly implemented in tests. Analyzes test coverage against specifications and identifies gaps.

**Input**: Optionally specify a change name and/or scope (e.g., `/opsx-verify add-auth` or `/opsx-verify add-auth groups 1-4`). If omitted, checks if it can be inferred from conversation context.
...
2. **Get specification files**
   ...
   Extract `contextFiles` to find spec file paths. Look in `specs/**/*.md` within the change directory. Read all spec files in the specified scope.
...
5. **Run test suite verification**
   ...
   Verify that all tests pass, test count matches expectations, and no skipped tests exist.
