...
The system SHALL automatically calculate total revenue to date as the sum of estimated values from all Won proposals linked to the Customer. This field SHALL be read-only.

#### Scenario: Proposal marked as Won
- **WHEN** a Proposal linked to a Customer is marked as Won with an estimated value
- **THEN** the Customer's total revenue to date is recalculated to include that amount
...
The system SHALL display all linked Proposals for a Customer in a dedicated proposals section on the detail page, ordered by date asked descending, showing title (as link to the proposal), status, estimated value, and expected close date.
