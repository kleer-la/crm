...
### Requirement: Sidebar uses slate-700 with indigo active indicator
The sidebar navigation SHALL use `bg-slate-700` as its background. The active navigation item SHALL be highlighted with a `bg-indigo-600` pill. Non-active items SHALL display as `text-slate-200` with `hover:bg-slate-600` hover state. The brand bar at the top SHALL use `bg-slate-800`.

#### Scenario: Active nav item
- **WHEN** the current page matches a navigation link
- **THEN** that link SHALL render with `bg-indigo-600 text-white` classes applied
